defmodule Simulator.Standard.WorkerActor do
  @moduledoc false

  import Simulator.Standard.Cell
  import Simulator.Standard.Position
  import Simulator.Standard.Printer
  ...
  def listen(cells_by_coords, neighbors_by_coords, signal_by_coords) do
    receive do
      ...
      {:start_iteration, iteration} ->
        plans =
          ...
          |> Enum.map(fn position ->
            create_plan(position, cells_by_coords, neighbors_by_coords)
          end)
      ...
      {:remote_consequences, iteration, consequences} ->
        ...
        distribute_signal(
          iteration,
          calculate_signal_updates(cells_by_coords, neighbors_by_coords, signal_by_coords)
        )
  ...
  def random_move(cell_position, cells_by_coords, neighbors_by_coords) do
    available_directions =
      ...
      |> Enum.filter(fn {_, position} ->
        cells_by_coords[position] == :empty
      end)
  ...
  def calculate_signal_updates(cells_by_coord, neighbors_by_coord, signal_by_coord) do
    ...
    |> Enum.map(fn coords ->
      {coords,
       calculate_signal_update(
         coords,
         cells_by_coord,
         neighbors_by_coord[coords],
         signal_by_coord
       )}
    end)
    |> Map.new()
  ...
  def calculate_signal_update(coords, cells_by_coord, cell_neighbors, signals_by_coord) do
    ...
    |> Enum.map(fn {direction, _direction_signal} ->
      {direction,
       calculate_signal_for_direction(
         direction,
         cells_by_coord,
         Map.get(cell_neighbors, direction, nil),
         signals_by_coord
       )}
    end)
    |> Map.new()
  ...
  """
  def calculate_signal_for_direction(
        direction,
        cells_by_coords,
        neighbor_coords,
        signal_by_coords
    ...
    cond do
          ...
          _ ->
            propagated_signal =
              ...
              |> Enum.map(fn neighbor_direction ->
                Map.get(signal_by_coords[neighbor_coords], neighbor_direction)
              end)
              |> Enum.sum()
      ...
      direction in [:top_right, :bottom_right, :bottom_left, :top_left] ->
        case neighbor_coords do
          ...
          _ ->
            Map.get(signal_by_coords[neighbor_coords], direction) +
              generate_signal(cells_by_coords[neighbor_coords])
        end
  ...
  def apply_signal_updates(old_signal_by_coord, signal_update_by_coord, cells_by_coords) do
    ...
    |> Enum.map(fn {coords, cell_signal} ->
      {coords,
       apply_signal_update(
         cell_signal,
         signal_update_by_coord[coords],
         signal_factor(cells_by_coords[coords])
       )}
    end)
    |> Map.new()
  end
  ...
  def apply_signal_update(old_cell_signal, cell_signal_update, cell_signal_factor) do
    ...
    |> Enum.map(fn {direction, signal} ->
      {direction,
       (signal + cell_signal_update[direction] * @signal_suppression_factor) *
         @signal_attenuation_factor * cell_signal_factor}
    end)
    |> Map.new()
  end
