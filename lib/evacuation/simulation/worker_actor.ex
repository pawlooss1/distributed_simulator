defmodule Simulator.Evacuation.WorkerActor do
  @moduledoc false

  import Nx.Defn
  import Simulator.Evacuation.Cell
  ...
  def listen(grid) do
    receive do
      ...
      {:start_iteration, iteration} ->
        plans = create_plans(iteration, grid)
#        IO.inspect(grid)
  ...
  """
  defnp create_plans(iteration, grid) do
    ...
    {_i, _j, _direction, signals, _grid} =
      while {i, j, direction = 1, signals = Nx.broadcast(Nx.tensor(0), {8}), grid},
        ...
        if can_move({x, y}, grid) do
          ...
          signals = Nx.put_slice(signals, Nx.broadcast(grid[i][j][-@infinity], {1}), [direction-1])
          {i, j, direction + 1, signals, grid}
        end
      end
  ...
  defnp create_plan_fire(i, j, grid, iteration) do
    if Nx.remainder(iteration, @fire_spreading_frequency) == Nx.tensor(0) do
      ...
      action = Nx.tensor([@fire])

      Nx.concatenate([direction, action])
    else
      Nx.tensor([0, 0, 0])
    end
  end
